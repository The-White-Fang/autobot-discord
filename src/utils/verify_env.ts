export default function verify_env() {
  ...
  if (!validate_env_var(app_id)) {
    console.error(
      "Missing or invalid DISCORD_APP_ID in environment variables.",
    );
    process.exit(1);
  }
  ...
  if (!validate_env_var(token)) {
    console.error(
      "Missing or invalid DISCORD_APP_TOKEN in environment variables.",
    );
    process.exit(1);
  }
  ...
  if (!validate_env_var(public_key)) {
    console.error(
      "Missing or invalid DISCORD_APP_PUBLIC_KEY in environment variables.",
    );
    process.exit(1);
  }
